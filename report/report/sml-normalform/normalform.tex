...
\begin{definition}[Generalisation of single pattern-body pairs, $|>$]\ \\
  ...
  \fref{eq:single-gen-2} holds when \fref{eq:single-gen-1} does not and
  \[
...
\item $p'_i$ and $p_j$ are equivalent. The pattern was generalised to one that
  ...
  (\Fref{def:alpha-equivalence-patexp}).
...
\fixme{Indsæt refernece til appendix hvor lortet kører in the wild!}
...
\fixme{Reference til appendixs (it works bitches).}
...
\section{Further generalisation}
...
\fref{sec:generalisiation}). We require the clauses to be alpha equivalent. It
...
\item[Simplification.] Clause bodies that have the same meaning can take many
  ...
  \fixme{Reference til afsnit om Simpset.}
...
Which is different in meaning to the second clause.
