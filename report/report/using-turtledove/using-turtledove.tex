\chapter{Using Turtledove}\label{chap:using-turtledove}
...
To test that the system is set up correctly the command ``\ttt{mlton
  Turtledove.mlb}'' can be run, and if it compiles without any errors,
everything should compile. Remember that MyLib must be installed before
attempting to compile any Turtledove related code.

The root code folder contains a \ttt{Makefile} that will build all the
