...
\begin{definition}[Alpha equivalence on expressions, $==a$]
  \begin{eqnarray}[rl]
    ...
    \sigma |- e_1e_2 &==a e_3e_4 \label{eq:alpha-exp} \\
    \sigma |- \lambda m_1 &==a \lambda m_2 \label{eq:alpha-match} \\
    \sigma |- c\ttt{(}e^1_1 \ttt{,} \ldots \ttt{,} e^1_n \ttt{)} &==a 
    c\ttt{(}e^2_1 \ttt{,} \ldots \ttt{,} e^2_n \ttt{)} \label{eq:alpha-con}
  \end{eqnarray}

where \fref{eq:alpha-var} holds iff 
\[
\sigma (v_1) = v_2 \lor v_1 = v_2
\]
and \fref{eq:alpha-exp} holds iff
\[
\sigma |- e_1 ==a e_3 \land \sigma |- e_2 ==a e_4
\]
and \fref{eq:alpha-match} holds iff alpha equivalence on matches holds \\
and \fref{eq:alpha-con} holds iff
\[
\sigma |- e^1_1 ==a e^1_n \land \ldots \land \sigma |- e^2_1 ==a e^2_n
\]
\end{definition}

Note that we write $t_1 ==a t_2$ to denote $[] |- t_1 ==a t_2$ and that two
terms are alpha equivalent if and only if they can be proven to be so using the
above rules.
...
the quotient set of patterns by alpha equivalence. Then we show that they indeed
are orderings.
...
\begin{lemma}[Partial ordering]\label{lem:pat-partial-orderings}
  ...
  on the equivalence classes of patterns modulo alpha equivalence
  ($pat_{/_{==a}}$).
\end{lemma}
...
\begin{proof}
  ...
  \begin{description}
  \item[Reflexivity.]
    Assume $p_1 ==a p_2$. We want to show $p_1 <=' p_2$.
    ...
    \fixme{Henvis til definition af alfaækvivalens.}
  ...
  \item[Antisymmetry.]
    Assume $p_1 <=' p_2$ and $p_2 <=' p_1$. We need to show $p_1 ==a p_2$.

    If either pattern is a variable then so is the other and alpha equivalence
    follows directly.
