...
where \fref{eq:alpha-var} holds if
\[
\sigma (v_1) = v_2 \lor \left( v_1 = v_2 \tnm{ and } \sigma(v_1),\sigma(v_2)
  \tnm{ is undefined} \right)
\]
and \fref{eq:alpha-exp} holds if
\[
\sigma |- e_1 ==a e_3 \land \sigma |- e_2 ==a e_4
\]
and \fref{eq:alpha-match} holds if alpha equivalence on matches holds \\
...
\begin{definition}[Alpha equivalence on matches, $==a$]
  \begin{eqnarray}[rl]
    \sigma |- \epsilon &==a \epsilon \label{eq:alpha-epsilon} \\
    \sigma |- p_1\ttt{.}e_1 &==a p_2\ttt{.}e_2 \land \sigma |- m_1 ==a m_2 \label{eq:alpha-pat-exp}
  \end{eqnarray}
where \fref{eq:alpha-pat-exp} holds if
\[
p_1 ==_{\sigma'} p_2 \land \sigma ++ \sigma' |- e_1 ==a e_2
\]
\end{definition}

We write $t_1 ==a t_2$ to denote $[] |- t_1 ==a t_2$.

Note that two terms are alpha equivalent if and only if they can be proven to be
so using the above rules.
